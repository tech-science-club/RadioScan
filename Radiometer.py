...
class Real_time_activity(MDScreen):         #------------> real time activity and displaying it on the screen
    ...
    def reading_data(self, dt):
        ...
        #self.temperature.append(self.read_data_t)      #----------> temperature and geotags might be stored as well
        ...
        #if len(self.coordinates) > 25:
        #    self.coordinates.pop(0)
        #if len(self.temperature) > 25:
    ...
    def anime(self, **kwargs):          #-----------> animation of picture in left bottom corner of the screen
        self.anim = Animation(opacity=1, duration=0.5) + Animation(opacity=0, duration=1)
        self.anim.repeat = True
        self.anim.start(self.Img)

class Bar(MDScreen):            # ----------> real time plotting
    ...
    def start_bar_plotting(self):
        try:
            self.Ard_Data = serial.Serial("com9", 9600)
    ...
    def update_points(self, args):
        ...
        self.plot2.points = [(i, self.y_axes[i]) for i in range(len(self.y_axes))]
